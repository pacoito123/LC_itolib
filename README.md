...
* **DetectRegion:**  An abstract region within which to detect or perform (non-allocating) searches for overlapping `Collider` instances belonging to objects of a certain type.
  ...
  * **WallBreaker:** Similar to `ConnectorMerger` (and it might be absorbed by it at some point), but for checking any blockers that are right in front of each other (within a certain tolerance), in order to disable both and create an illegitimate, unsanctioned `Doorway`.
