...
namespace itolib.Behaviours.Grabbables
{
    ...
    public class ItemAudible : NetworkBehaviour
    {
        ...
        private void Awake()
        {
            ...
            {
                Plugin.Logger.LogWarning($"Could not find IEventfulItem for ItemAudible component in GameObject '{gameObject.name}'.");
                enabled = false;
        ...
        public void PlayNetworkedAudio(int clip, int clipFar = -1, float volume = 1.0f, float pitch = 1.0f, float loudness = 0.0f)
        {
            ...
            if (IsSpawned)
            {
                PlayNetworkedAudioRpc(clip, clipFar, volume, pitch, loudness);
            }
        }
        ...
        private void PlayNetworkedAudioRpc(int clip, int clipFar, float volume, float pitch, float loudness)
        {
            PlayAudioLocal(clip, clipFar, volume, pitch, loudness);
        }
